...
void inline CallbackWrapper::post_callback_tag_update(oop o,
                                                      ...
                                                      jlong old_tag,
                                                      jlong obj_tag){
  // The semantic is slightly changed.
  // Initially it will always set the tag.
  if (old_tag != obj_tag) tag_map->compare_and_swap_tag(o, obj_tag, old_tag);
}
...
jlong JvmtiTagMap::compare_and_swap_tag(oop o, jlong tag, jlong compare) {
  // setting tag to 0 means remove tag
  volatile jlong* tag_addr = ((volatile jlong *) o) + 2;
  jlong res = Atomic::cmpxchg(tag, tag_addr, compare);
...
void JvmtiTagMap::set_tag(jobject object, jlong tag) {
  ...
  do {
      old_tag = o->get_tag();
  } while (compare_and_swap_tag(o, tag, old_tag) != old_tag);
}
